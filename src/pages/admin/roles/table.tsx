...
import { Role, deleteRole } from './api';
...
const RoleTable: React.FC<TableProps> = ({
  data,
  loading = false,
  onNavigate,
  onDelete,
  ...
  const renderDeleteModal = () => {
    if (!deleteModalVisible || !roleToDelete) return null;
    ...
    return (
      <>
        ...
        <div className="bg-[var(--surface-1)] rounded-lg shadow-sm border border-[var(--glass-border)] overflow-hidden">
          {loading ? (
            <div className="py-12 text-center text-[var(--text-secondary)]">{t.loadingRoles}</div>
          ) : data.length === 0 ? (
              ...
              {data.map((role) => (
                ...
                >
                  ...
                  <div className="space-y-2 text-xs text-[var(--text-secondary)]">
                    {role.description && (
                      <div>
                        <span className="text-[var(--text-primary)]">{role.description}</span>
                      </div>
                    )}
                    <div className="text-xs">
                      {t.updatedAt}: {formatDate(role.updatedAt)}
                    </div>
                  </div>
                  <div className="flex items-center justify-end gap-2 mt-3">
                    <button
                      onClick={() => onNavigate?.('view', role)}
                    ...
                    <button
                      onClick={() => onNavigate?.('form', { ...role, mode: 'edit' })}
                      className="p-2 rounded hover:bg-[var(--glass-bg)] transition-colors focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2"
                      title="Edit"
                      aria-label="Edit"
                    >
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={4} className="px-[18px] py-12 text-center text-[var(--text-secondary)] text-sm">
                    {t.loadingRoles}
                  </td>
                </tr>
              ) : data.length === 0 ? (
